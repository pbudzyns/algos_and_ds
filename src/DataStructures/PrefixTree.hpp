...
class PrefixTree
{
  ...
  private:
    prefix_tree_imp::TreeNode* m_Root;

    void getSuffixes(std::vector<std::string>* suffixes, std::string prefix,
                     prefix_tree_imp::TreeNode* root);
    void deleteNode(prefix_tree_imp::TreeNode* node);
};
