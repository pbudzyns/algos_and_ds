 ...
 *
 * @details Heap implementation using DynamicArray as an underlying
 * container. Heap is a complete binary tree that allows to retrieve minimum
 * (MinHeap) or maximum (MaxHeap) in constant time `O(1)`. Inserting and
 * deleting elements from Heap take `O(log n)` time.
 *
 * Types stored in the Heap should implement compare operators (at least `<`
 * for default implementation).
 *
 * By default this template creates MinHeap. To create MaxHeap provide
 * proper comparison function to the constructor.
 *
 * Example usage:
 * @code
 * Heap<int> heap;
...
class Heap
{
  ...
  private:
    ...
    size_type getLeftChild(size_type index) const
    {
        return 2 * index + 1;
    }
    size_type getRightChild(size_type index) const
    {
        return 2 * index + 2;
    }
    size_type getParent(size_type index) const
    {
        return (index - 1) / 2;
    }
